...
If you're searching for **document ingestion**, **text extraction**, **PDF extraction**, or a **document extractor** for books and technical content, this project gives you dual extraction (native DOCX + PDF and pytesseract OCR), side-by-side comparison, and a web-based admin console.
...
- **Dual extractor**: Parse both DOCX (native) and PDF (native + pytesseract OCR) and compare results
...
| **PDF extractor** | Native PyMuPDF + pytesseract OCR (Tesseract) on screenshots or rendered pages |
...
- **Extraction**: PyMuPDF (PDF), pytesseract/Tesseract (OCR), python-docx (DOCX)
...
- **Optional**: Tesseract (for OCR extraction and page accuracy; install via brew/apt)
...
### 4. Optional: Tesseract OCR (extraction and page accuracy)
...
├── Docs/             # Business requirements, setup guides (e.g. nginx)
...
E2E UI tests use [Playwright](https://playwright.dev/). From the frontend directory:
...
- **test:e2e** — run all E2E tests (Playwright starts the Vite dev server automatically).
- **test:e2e:ui** — open the Playwright UI for debugging.
- **test:e2e:headed** — run tests in a visible browser.

For tests that call the real API (e.g. document list, upload), the backend must be running on port 8889.
